...
// print_r("<pre>");
// print_r($json_latih[0]['tfidf']);
// print_r("<br>");

// print_r("<hr><h1>SVD datalatih</h1><br>");
$d_datalatih[0] = count($json_latih[0]['tfidf']);
$t_datalatih[0] = count(matrixTranspose($json_latih[0]['tfidf']));
if ($t_datalatih[0] > $d_datalatih[0]) {
	$matrix_datalatih[0] = matrixTranspose($json_latih[0]['tfidf']);
} elseif ( $d_datalatih[0] >= $t_datalatih[0]) {
	$matrix_datalatih[0] = $json_latih[0]['tfidf'];
}
print_r("TERM DATA LATIH = ".$t_datalatih[0]);
print_r("<br>");
print_r("DOKUMEN DATA LATIH = ".$d_datalatih[0]);

if (!empty($matrix_datalatih[0])) {
	$svd_datalatih[0] = SVD($matrix_datalatih[0]);
}

$ini_k[0] = intval($svd_datalatih[0]['K']*5/100);
$ini_k[1] = intval($svd_datalatih[0]['K']*10/100);
$ini_k[2] = intval($svd_datalatih[0]['K']*15/100);
$ini_k[3] = intval($svd_datalatih[0]['K']*20/100);
$ini_k[4] = intval($svd_datalatih[0]['K']*25/100);
$ini_k[5] = intval($svd_datalatih[0]['K']*30/100);
$ini_k[6] = intval($svd_datalatih[0]['K']*35/100);
$ini_k[7] = intval($svd_datalatih[0]['K']*40/100);
$ini_k[8] = intval($svd_datalatih[0]['K']*45/100);
$ini_k[9] = intval($svd_datalatih[0]['K']*50/100);
$ini_k[10] = intval($svd_datalatih[0]['K']*55/100);
$ini_k[11] = intval($svd_datalatih[0]['K']*60/100);
$ini_k[12] = intval($svd_datalatih[0]['K']*65/100);
$ini_k[13] = intval($svd_datalatih[0]['K']*70/100);
$ini_k[14] = intval($svd_datalatih[0]['K']*75/100);
$ini_k[15] = intval($svd_datalatih[0]['K']*80/100);
$ini_k[16] = intval($svd_datalatih[0]['K']*85/100);
$ini_k[17] = intval($svd_datalatih[0]['K']*90/100);
$ini_k[18] = intval($svd_datalatih[0]['K']*95/100);
$ini_k[19] = intval($svd_datalatih[0]['K']);
// print_r("<pre>");
// print_r($ini_k);

foreach ($ini_k as $key => $value) {
	$svd_datalatih_red[0][$key] = array();
	$svd_datalatih_red[0][$key]['sp'] = matrixConstruct($svd_datalatih[0]['S'], $value, $value);
	$svd_datalatih_red[0][$key]['up'] = matrixConstruct($svd_datalatih[0]['U'], count($svd_datalatih[0]['U']), $value);
	$svd_datalatih_red[0][$key]['vtp'] = matrixConstruct(matrixTranspose($svd_datalatih[0]['V']), $value, count($svd_datalatih[0]['V']));
	$newArr[0][$key] = Array();
	foreach ($svd_datalatih_red[0][$key]['sp']  as $k => $val) {
		$r = Array();
		foreach ($val as $v) {
			array_push($r, floatval($v) == 0.0 ? 0 : floatval($v));
		}
		array_push($newArr[0][$key], $r);
	}
	$spinvers_datalatih[0][$key] = invert($newArr[0][$key]);
	// echo "<hr><pre><h1>LATENT SEMANTIC ANALYSIS LATIH</h1>";
	if ($t_datalatih[0] > $d_datalatih[0]) {
		// print_r("<br> disini buat t > d rumusnya q=qTUkS-1");
		// $red[$key] = $svd_datalatih_red[0][$key]['up'];
		$red[$key] = perkalian_matrix($svd_datalatih_red[0][$key]['up'], $spinvers_datalatih[0][$key]);
	} elseif ( $d_datalatih[0] >= $t_datalatih[0]) {
		// print_r("<br>disini buat d > t rumusnya q=qTVTkS-1");
		// $red[$key] = matrixTranspose($svd_datalatih_red[0][$key]['vtp']);
		$red[$key] = perkalian_matrix(matrixTranspose($svd_datalatih_red[0][$key]['vtp']), $spinvers_datalatih[0][$key]);
	}
	// REDUKSI PADA DATA LATIH
	for ($x=0; $x < $d_datalatih[0] ; $x++) {
		$tf_datalatih_red[0][$key][$x] = perkalian_matrix(array($json_latih[0]['tfidf'][$x]), $red[$key]);
		foreach ($tf_datalatih_red[0][$key] as $k => $value) {
			foreach ($value as $kunci => $val) {
				$svd_datalatih[0][$key]['red'][$k] = $val;
			}
		}
	}
	//REDUKSI PADA DATA UJI
	$x=0;
	foreach ($json_uji[0] as $k => $value) {
		$tf_uji_red[0][$key][$k] = perkalian_matrix(array($json_uji[0][$k]['tf']),$red[$key]);
		foreach ($tf_uji_red[0][$key] as $ke => $value) {
			foreach ($value as $kunci => $val) {
				$svd_datauji[0][$key][$ke] = $val;
			}
		}
	}
	$datalatihred[0][$key] = new Labeled($svd_datalatih[0][$key]['red'], $json_latih[0]['label']);
	$dataujired[0][$key] = new Labeled($svd_datauji[0][$key], $labeluji[0]);
...
// //===========================================================================================================================================//
	...
	$estimator->train($datalatihred[0][$key]);
	$predictions[0][$key] = $estimator->predict($dataujired[0][$key]);
	$results[0][$key] = $report->generate($predictions[0][$key], $labeluji[0]);
	$akurasi[0][$key] = $results[0][$key]['overall']['accuracy'];
	$fmeasure[0][$key] = $results[0][$key]['overall']['f1_score'];
	$presisi[0][$key] = $results[0][$key]['overall']['precision'];
	$recall[0][$key] = $results[0][$key]['overall']['recall'];
	print_r("<br>AKURASI KE-   " . 0 . " = " . $results[0][$key]['overall']['accuracy']);
	print_r("<br>F-MEASURE KE- " . 0 . " = " . $results[0][$key]['overall']['f1_score']);
	print_r("<br>PRECISION KE- " . 0 . " = " . $results[0][$key]['overall']['precision']);
	print_r("<br>RECALL KE-    " . 0 . " = " . $results[0][$key]['overall']['recall']);
	print_r("<br><br>");
