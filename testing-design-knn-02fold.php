...
//write json to file
if (file_put_contents("json/datalatih-knn-2-fold.json", $json_datalatih))
...
//write json to file
if (file_put_contents("json/datauji-knn-2-fold.json", $json_datauji))
...
print_r('<pre><h3>PENGUJIAN 02-FOLD KNearestNeighbors K=5 </h3>');
...
// 	foreach($value as $kunci => $val){
// 		$data_kandidat[$key][$kunci] = mysqli_query($connect, "SELECT * FROM `kfold_datalatih_svm` WHERE label = '$val' ");
...
// 				$nilaicossim[$key][$kunci][$i] = cosinesimilarity($tf_uji[$key][$kunci], matrixTranspose($tfidf_kandidat[$key][$i]));
