...
$data_uji = file_get_contents("json/datauji_knn.json");
$json_uji = json_decode($data_uji, TRUE);
print_r("<pre>");
foreach ($json_uji[2] as $key => $value) {
	$labeluji[2][$key] = $value['label'];
}
// print_r($labeluji);

$data_latih = file_get_contents("json/datalatih_knn.json");
$json_latih = json_decode($data_latih, TRUE);
// print_r("<pre>");
// print_r($json_latih[2]['tfidf']);
// print_r("<br>");

// print_r("<hr><h1>SVD datalatih</h1><br>");
$d_datalatih[2] = count($json_latih[2]['tfidf']);
$t_datalatih[2] = count(matrixTranspose($json_latih[2]['tfidf']));
if ($t_datalatih[2] > $d_datalatih[2]) {
	$matrix_datalatih[2] = matrixTranspose($json_latih[2]['tfidf']);
} elseif ( $d_datalatih[2] >= $t_datalatih[2]) {
	$matrix_datalatih[2] = $json_latih[2]['tfidf'];
}
print_r("TERM DATA LATIH = ".$t_datalatih[2]);
print_r("<br>");
print_r("DOKUMEN DATA LATIH = ".$d_datalatih[2]);

if (!empty($matrix_datalatih[2])) {
	$svd_datalatih[2] = SVD($matrix_datalatih[2]);
}

$ini_k[0] = intval($svd_datalatih[2]['K']*5/100);
$ini_k[1] = intval($svd_datalatih[2]['K']*10/100);
$ini_k[2] = intval($svd_datalatih[2]['K']*15/100);
$ini_k[3] = intval($svd_datalatih[2]['K']*20/100);
$ini_k[4] = intval($svd_datalatih[2]['K']*25/100);
$ini_k[5] = intval($svd_datalatih[2]['K']*30/100);
$ini_k[6] = intval($svd_datalatih[2]['K']*35/100);
$ini_k[7] = intval($svd_datalatih[2]['K']*40/100);
$ini_k[8] = intval($svd_datalatih[2]['K']*45/100);
$ini_k[9] = intval($svd_datalatih[2]['K']*50/100);
$ini_k[10] = intval($svd_datalatih[2]['K']*55/100);
$ini_k[11] = intval($svd_datalatih[2]['K']*60/100);
$ini_k[12] = intval($svd_datalatih[2]['K']*65/100);
$ini_k[13] = intval($svd_datalatih[2]['K']*70/100);
$ini_k[14] = intval($svd_datalatih[2]['K']*75/100);
$ini_k[15] = intval($svd_datalatih[2]['K']*80/100);
$ini_k[16] = intval($svd_datalatih[2]['K']*85/100);
$ini_k[17] = intval($svd_datalatih[2]['K']*90/100);
$ini_k[18] = intval($svd_datalatih[2]['K']*95/100);
$ini_k[19] = intval($svd_datalatih[2]['K']);
// print_r("<pre>");
// print_r($ini_k);

foreach ($ini_k as $key => $value) {
	$svd_datalatih_red[2][$key] = array();
	$svd_datalatih_red[2][$key]['sp'] = matrixConstruct($svd_datalatih[2]['S'], $value, $value);
	$svd_datalatih_red[2][$key]['up'] = matrixConstruct($svd_datalatih[2]['U'], count($svd_datalatih[2]['U']), $value);
	$svd_datalatih_red[2][$key]['vtp'] = matrixConstruct(matrixTranspose($svd_datalatih[2]['V']), $value, count($svd_datalatih[2]['V']));
	$newArr[2][$key] = Array();
	foreach ($svd_datalatih_red[2][$key]['sp']  as $k => $val) {
		$r = Array();
		foreach ($val as $v) {
			array_push($r, floatval($v) == 0.0 ? 0 : floatval($v));
		}
		array_push($newArr[2][$key], $r);
	}
	$spinvers_datalatih[2][$key] = invert($newArr[2][$key]);
	// echo "<hr><pre><h1>LATENT SEMANTIC ANALYSIS LATIH</h1>";
	if ($t_datalatih[2] > $d_datalatih[2]) {
		// print_r("<br> disini buat t > d rumusnya q=qTUkS-1");
		// $red[$key] = $svd_datalatih_red[2][$key]['up'];
		$red[$key] = perkalian_matrix($svd_datalatih_red[2][$key]['up'], $spinvers_datalatih[2][$key]);
	} elseif ( $d_datalatih[2] >= $t_datalatih[2]) {
		// print_r("<br>disini buat d > t rumusnya q=qTVTkS-1");
		// $red[$key] = matrixTranspose($svd_datalatih_red[2][$key]['vtp']);
		$red[$key] = perkalian_matrix(matrixTranspose($svd_datalatih_red[2][$key]['vtp']), $spinvers_datalatih[2][$key]);
	}
	// REDUKSI PADA DATA LATIH
	for ($x=0; $x < $d_datalatih[2] ; $x++) {
		$tf_datalatih_red[2][$key][$x] = perkalian_matrix(array($json_latih[2]['tfidf'][$x]), $red[$key]);
		foreach ($tf_datalatih_red[2][$key] as $k => $value) {
			foreach ($value as $kunci => $val) {
				$svd_datalatih[2][$key]['red'][$k] = $val;
			}
		}
	}
	//REDUKSI PADA DATA UJI
	$x=0;
	foreach ($json_uji[2] as $k => $value) {
		$tf_uji_red[2][$key][$k] = perkalian_matrix(array($json_uji[2][$k]['tf']),$red[$key]);
		foreach ($tf_uji_red[2][$key] as $ke => $value) {
			foreach ($value as $kunci => $val) {
				$svd_datauji[2][$key][$ke] = $val;
			}
		}
	}
	$datalatihred[2][$key] = new Labeled($svd_datalatih[2][$key]['red'], $json_latih[2]['label']);
	$dataujired[2][$key] = new Labeled($svd_datauji[2][$key], $labeluji[2]);
...
// //===========================================================================================================================================//
	...
	$estimator->train($datalatihred[2][$key]);
	$predictions[2][$key] = $estimator->predict($dataujired[2][$key]);
	$results[2][$key] = $report->generate($predictions[2][$key], $labeluji[2]);
	$akurasi[2][$key] = $results[2][$key]['overall']['accuracy'];
	$fmeasure[2][$key] = $results[2][$key]['overall']['f1_score'];
	$presisi[2][$key] = $results[2][$key]['overall']['precision'];
	$recall[2][$key] = $results[2][$key]['overall']['recall'];
	print_r("<br>AKURASI KE-   " . 2 . " = " . $results[2][$key]['overall']['accuracy']);
	print_r("<br>F-MEASURE KE- " . 2 . " = " . $results[2][$key]['overall']['f1_score']);
	print_r("<br>PRECISION KE- " . 2 . " = " . $results[2][$key]['overall']['precision']);
	print_r("<br>RECALL KE-    " . 2 . " = " . $results[2][$key]['overall']['recall']);
	print_r("<br><br>");
	// print_r($results[2][$key]);
